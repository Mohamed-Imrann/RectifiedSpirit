...
def search_published_series(query, limit=10):
  """
  Searches for published series by title or key using regex.
  Returns a list of matching series documents.
  """
  # Create a regex pattern that matches the query anywhere in the title or key
  # Case-insensitive search
  regex_pattern = re.compile(f".*{re.escape(query)}.*", re.IGNORECASE)
  
  # Search for published series where title or _id matches the regex
  filter_query = {
      "published": True,
      "$or": [
          {"title": {"$regex": regex_pattern}},
          {"_id": {"$regex": regex_pattern}}
      ]
  }
  
  return list(series_collection.find(filter_query).limit(limit))
