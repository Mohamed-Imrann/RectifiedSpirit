...
class JoinReqs:
    def __init__(self):
        ...
        if DATABASE_URL:
            ...
            # request tracking collections (per channel id)
            self.col1 = self.db[str(REQ_CHANNEL_ONE)] if REQ_CHANNEL_ONE else None
            self.col2 = self.db[str(REQ_CHANNEL_TWO)] if REQ_CHANNEL_TWO else None
            self.col3 = self.db[str(REQ_CHANNEL_THREE)] if REQ_CHANNEL_THREE else None

            # chat id storage collections
