...
async def get_link_key_by_token(user_id: int, token: str):
    """Return real link_key for a token.

    Primary: match (user_id + token) and not expired.
    Fallback: if not found, match token-only (still not expired).
    This prevents 'KEEP pending' when earlier code stored token under a different user_id field.
    """
    ...
    # 2) fallback: token-only (still respect expiry)
    doc = temp_tokens.find_one({
        "token": str(token),
        "expires_at": {"$gt": datetime.utcnow()},
    })
