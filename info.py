...
NEW_DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://SpidySeries:SpidySeries@cluster0.xreosjj.mongodb.net/?retryWrites=true&w=majority")
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
# For migration
DATABASE_URI = "mongodb+srv://sp:sp@cluster0.eh3zdcl.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
...
# FSUB 
auth_channel = environ.get('AUTH_CHANNEL' "-1002962062109")
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
# Set to False inside the bracket if you don't want to use Request Channel else set it to Channel ID
REQ_CHANNEL = environ.get("REQ_CHANNEL", -1002962062109)
JOIN_REQS_DB = environ.get("JOIN_REQS_DB", DATABASE_URI)
LIMIT = 2000
pgHost = environ.get("POSTGRES_HOST", "spidy_bots_postgres")  # Docker service name
