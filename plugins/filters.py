...
from database.crazy_db import (
  db, add_series, get_series_by_key, update_series_field, add_or_update_language,
  ...
  get_poster_file_id, update_poster_file_id, publish_series, get_series, search_published_series
)
from database.gfilters_mdb import (
  find_gfilter,
  get_gfilters
)
from utils import get_message_id, get_messages_in_range, delete_messages_from_user_chat, get_poster, find_most_similar_title, temp, chunk_buttons
...
async def _new_series_command_logic(client: Client, message: Message, query: str):
  ...
  imdb_results = get_poster(query, bulk=True)
  ...
  for keyword in reversed(sorted(keywords, key=len)):
      ...
      if re.search(pattern, name, flags=re.IGNORECASE):
          ...
          try:
              ...
              logger.exception(e)
          return True # Indicate that a global filter was handled
  return False # Indicate no global filter was handled

async def series_filter(client, message):
  text = message.text.strip()
  
  matching_series = db.search_published_series(text, limit=10)
  ...
  if not series_data:
      ...
      close_matches_titles = find_most_similar_title(text, matching_series_titles) # Using find_most_similar_title
      
      if close_matches_titles:
          buttons = []
          # find_most_similar_title returns a single best match, so we need to adjust
          # If you want multiple close matches, you'd use difflib.get_close_matches directly
          # For now, assuming find_most_similar_title returns the best single match
...
async def consolidated_message_handler(client: Client, message: Message):
  user_id = message.from_user.id
  ...
  if is_admin:
      ...
      if message.text and message.text.startswith('/'):
          ...
          if command == "/newseries":
              ...
              # If it's an unknown command for admin, still check for state-based text input
              if message.text:
                  if current_state == "AWAITING_LANGUAGE_INPUT":
                      ...
                      # If no state matches, it's an unhandled admin text message
                      pass # Or send a "command not recognized" message
      ...
      # Handle admin state-based text/media input if not a command
      elif message.text:
          if current_state == "AWAITING_LANGUAGE_INPUT":
              await _process_language_input(client, message, message.text.strip())
          elif current_state == "AWAITING_SEASON_INPUT":
              ...
              pass # Unhandled admin text message
      elif message.photo or message.video or message.document:
          if current_state == "AWAITING_SERIES_POSTER":
              await _process_poster_input(client, message, "series")
          elif current_state == "AWAITING_LANGUAGE_POSTER":
...
async def consolidated_callback_handler(client: Client, query: CallbackQuery):
  ...
  if user_id in ADMINS:
      ...
      # Dispatch based on admin action
      if action.startswith("tmdb_select") or action.startswith("imdb_select"):
          ...
          await query.answer("Unknown admin action or invalid state.", show_alert=True)
          logger.warning(f"Unhandled admin callback: {data} from user {user_id}")
  else:
      # Handle non-admin callbacks (public filters)
      if data == "pages":
          await query.answer()
          return
