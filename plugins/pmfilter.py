...
from database.crazy_db import (
  get_series, get_series_by_key, get_languages, get_seasons, get_qualities, get_quality_link, get_specific_poster # Import get_specific_poster
)
...
def get_series_poster_for_user(series_key, language_name=None, season_name=None):
  """
  Retrieves the most specific poster for a series, language, or season.
  Falls back to series poster, then NO_POSTER_FOUND_IMG.
  """
  poster_source = get_specific_poster(series_key, language_name, season_name)
  if poster_source:
      ...
      logger.info(f"No specific poster found for series {series_key}. Using default.")
      return NO_POSTER_FOUND_IMG
...
async def cb_handler(client, query: CallbackQuery):
  ...
  if data == "pages":
      ...
      if series_data and series_data.get('published', False):
          ...
          poster_to_use = get_series_poster_for_user(series_key, language_name=language_name) # Use language-specific poster
      ...
      if series_data and series_data.get('published', False):
          ...
          poster_to_use = get_series_poster_for_user(series_key, language_name=language_name, season_name=season_name) # Use season-specific poster
          ...
          try:
              ...
              logger.error(f"Error editing message media for quality details: {e}")
              await query.message.edit_text("An error occurred while fetching quality details.")
