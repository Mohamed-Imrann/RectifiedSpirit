...
async def media_selection_callback(client: Client, callback_query: CallbackQuery):
  ...
  if not movie_details:
      ...
      try:
          ...
          logger.warning(f"Admin {user_id}: Failed to edit message (ID: {main_message_id}): {e}. Sending new message.")
          new_msg = await client.send_message(user_id, f"Failed to retrieve {source.upper()} data. Please try again.")
          temp_admin_data[user_id]["main_message_id"] = new_msg.id # Update stored message ID
      return
      ...
      if not add_series(series_data): # Attempt to add, check if successful
          ...
          if not series_data: # If still not found, something is wrong
              ...
              try:
                  ...
                  logger.warning(f"Admin {user_id}: Failed to edit message (ID: {main_message_id}): {e}. Sending new message.")
                  new_msg = await client.send_message(user_id, "Failed to create or load series. Please try again.")
                  temp_admin_data[user_id]["main_message_id"] = new_msg.id # Update stored message ID
              return
  ...
  if not series_data: # Should not happen if add_series was successful or existing_series was found
      ...
      try:
          ...
          logger.warning(f"Admin {user_id}: Failed to edit message (ID: {main_message_id}): {e}. Sending new message.")
          new_msg = await client.send_message(user_id, "Failed to retrieve series data after initial setup. Please try again.")
          temp_admin_data[user_id]["main_message_id"] = new_msg.id # Update stored message ID
      return
...
async def local_series_selection_callback(client: Client, callback_query: CallbackQuery):
  ...
  if len(data_parts) > 1 and len(data_parts[1]) == 36 and '-' in data_parts[1]: # Basic UUID check
      ...
      if user_id not in temp_admin_data or unique_id not in temp_admin_data[user_id]:
          ...
          try:
              ...
              logger.warning(f"Admin {user_id}: Failed to delete message (ID: {callback_query.message.id}): {e}. Skipping deletion.")
          return
      stored_data = temp_admin_data[user_id].pop(unique_id)
  ...
  if not series_data:
      ...
      try:
          ...
          logger.warning(f"Admin {user_id}: Failed to edit message (ID: {main_message_id}): {e}. Sending new message.")
          new_msg = await client.send_message(user_id, "Selected series not found in database. It might have been deleted.")
          temp_admin_data[user_id]["main_message_id"] = new_msg.id # Update stored message ID
      return
...
async def publish_series_callback(client: Client, callback_query: CallbackQuery):
  ...
  try:
      await client.edit_message_text(
          chat_id=user_id,
          message_id=main_message_id,
          text="Do you want to publish this series? All empty languages, seasons, and qualities will be removed automatically. You can re-edit and re-publish this series later.",
  ...
  if publish_success:
      ...
      try:
          await client.edit_message_text(
              chat_id=user_id,
              message_id=main_message_id,
              text="Published Successfully! This series is now live and can be re-edited and re-published."
          )
  ...
  if not languages:
      ...
      if edit_message:
          try:
              ...
              logger.error(f"User {user_id} in chat {chat_id}: Another error editing message: {e}")
              new_msg = await client.send_message(message.chat.id, "No languages available for this series yet.")
