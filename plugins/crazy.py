...
def build_keyboard(items, category):
    """Build keyboard with max 3 columns pattern"""
    from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    
    # Create buttons for items
    buttons = [InlineKeyboardButton(text=item, callback_data=f"remove:{category}:{item}") for item in items]
    
    # Add + button
    buttons.append(InlineKeyboardButton(text="+", callback_data=f"add:{category}"))
    
    # Create rows with max 3 buttons per row
    rows = []
...
def get_keyboard(category, data_from_db):
    """Get keyboard for any category"""
    return build_keyboard(data_from_db, category)
...
async def send_new_series_details(client: Client, user_id: int, series_data: dict, message_id: int = None):
    ...
    buttons = [
        InlineKeyboardButton("✏️ Edit", callback_data="edit_series"),
...
async def send_new_series_editing_main(client: Client, user_id: int, series_key: str, message_id: int = None):
    ...
    buttons = [
        ...
        InlineKeyboardButton("🖼️ Poster", callback_data="edit_poster"),
        InlineKeyboardButton("📤 Publish", callback_data="publish_series"),
        InlineKeyboardButton("⬅️ Back", callback_data="back_to_details")
    ]
...
async def send_language_management(client: Client, user_id: int, series_key: str, message_id: int):
    ...
    # Build keyboard
    keyboard = get_keyboard("language", language_names)
...
async def send_season_management(client: Client, user_id: int, series_key: str, language_name: str, message_id: int):
    ...
    # Build keyboard
    keyboard = get_keyboard("season", season_names)
...
async def send_quality_management(client: Client, user_id: int, series_key: str, language_name: str, season_name: str, message_id: int):
    ...
    # Build keyboard
    keyboard = get_keyboard("quality", quality_names)
...
async def newui_callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    # Handle add/remove callbacks for languages, seasons, and qualities
        ...
        if category == "language":
            await callback_query.answer(f"Removing {item_name}...")
            
            if delete_language(series_key, item_name):
                await callback_query.answer(f"Removed {item_name} successfully.")
                await send_language_management(client, user_id, series_key, main_message_id)
            else:
            ...
            if delete_season(series_key, language_name, item_name):
                await callback_query.answer(f"Removed {item_name} successfully.")
            ...
            if delete_quality(series_key, language_name, season_name, item_name):
                await callback_query.answer(f"Removed {item_name} successfully.")
...
async def process_poster_input(client: Client, message: Message):
    ...
    series_data = get_series_by_key(series_key)
    if series_data:
        await send_new_series_editing_main(client, user_id, series_data, main_message_id)
...
async def process_codec_input(client: Client, message: Message, codec: str):
    ...
    # Create link key
    # Remove -100 from channel ID if present
    first_channel_id_str = str(first_channel_id).replace("-100", "")
    last_channel_id_str = str(last_channel_id).replace("-100", "")
    link_key = f"get_{first_channel_id_str}_{first_message_id}_{last_message_id}"
