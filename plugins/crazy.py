...
from database.crazy_db import (
    add_series, get_series_by_key, add_or_update_language, # Removed update_series_field
    ...
    get_poster_file_id, update_poster_file_id, publish_series, get_all_series, get_specific_poster, # Corrected get_series to get_all_series
...
async def send_main_series_message(client: Client, user_id: int, series_data: dict, message_id: int = None):
    ...
    poster_file_id = await get_poster_file_id(series_key) or NO_POSTER_FOUND_IMG # Await the async function
...
async def clone_series_command(client: Client, message: Message):
    ...
    existing_series_by_new_key = await get_series_by_key(new_series_key) # Await the async function
    ...
    original_series_data = await get_series_by_key(original_query) # Await the async function
    if not original_series_data:
        # Try searching by title if not found by key
        all_series = await get_all_series() # Await the async function
        for s in all_series:
    ...
    if await add_series(cloned_series_data): # Await the async function
...
async def edit_series_command(client: Client, message: Message):
    ...
    series_data = await get_series_by_key(series_key_lookup) # Await the async function
    ...
    temp_msg = await message.reply_photo(
        photo=await get_poster_file_id(series_data['_id']) or NO_POSTER_FOUND_IMG, # Await the async function
...
async def media_selection_callback(client: Client, callback_query: CallbackQuery):
    ...
    if source == 'tmdb':
        ...
        movie_details = get_poster(media_id, id=True) # get_poster is not async, so no await
    ...
    existing_series = await get_series_by_key(series_key) # Await the async function
    if existing_series:
        ...
        if not await add_series(series_data): # Await the async function # Attempt to add, check if successful
            await callback_query.answer("Failed to add new series (might already exist). Loading existing series.", show_alert=True)
            series_data = await get_series_by_key(series_key) # Await the async function # Re-fetch if insertion failed due to duplicate
    ...
    if poster_file_id:
        await update_poster_file_id(series_key, poster_file_id) # Await the async function
        ...
        await update_poster_file_id(series_key, NO_POSTER_FOUND_IMG) # Await the async function
...
async def process_language_input(client: Client, message: Message, language_name: str):
    ...
    if await add_or_update_language(series_key, language_name): # Await the async function
...
async def process_season_input(client: Client, message: Message, season_name: str):
    ...
    if await add_or_update_season(series_key, language_name, season_name): # Await the async function
        confirmation_msg = await client.send_message(
            chat_id=user_id,
...
async def process_quality_input(client: Client, message: Message, quality_name: str):
    ...
    if await add_or_update_quality(series_key, language_name, season_name, quality_name, "PENDING_LINK"): # Await the async function
        confirmation_msg = await client.send_message(
            chat_id=user_id,
...
async def process_codec_input(client: Client, message: Message, codec: str):
    ...
    if await add_or_update_quality(series_key, language_name, season_name, quality_name, link_key, codec): # Await the async function
...
async def process_poster_input(client: Client, message: Message, level: str):
    ...
    if new_poster_file_id:
        if level == "series":
            await update_poster_file_id(series_key, new_poster_file_id) # Await the async function
            await processing_msg.edit_text("Series poster updated successfully.")
            new_main_msg_id = await send_main_series_message(client, user_id, await get_series_by_key(series_key), main_message_id) # Await get_series_by_key
            if new_main_msg_id: temp_admin_data[user_id]["main_message_id"] = new_main_msg_id
            temp_admin_data[user_id]["state"] = "SERIES_DETAILS_VIEW"
        elif level == "language":
            await add_or_update_language(series_key, language_name, new_poster_file_id) # Await the async function
            await processing_msg.edit_text("Language poster updated successfully.")
            await send_language_management_message(client, user_id, series_key, main_message_id)
            temp_admin_data[user_id]["state"] = "MANAGE_LANGUAGES"
        elif level == "season":
            await add_or_update_season(series_key, language_name, season_name, new_poster_file_id) # Await the async function
...
async def process_edit_series_text(client: Client, message: Message, input_text: str):
    ...
    if updates:
        for field, value in updates.items():
            # Use update_series to update individual fields
            await update_series(series_key, {field: value}) # Await the async function
...
async def delete_language_callback(client: Client, callback_query: CallbackQuery):
    ...
    if await delete_language(series_key, language_name): # Await the async function
...
async def delete_season_callback(client: Client, callback_query: CallbackQuery):
    ...
    if await delete_season(series_key, language_name, season_name): # Await the async function
...
async def delete_quality_callback(client: Client, callback_query: CallbackQuery):
    ...
    if await delete_quality(series_key, language_name, season_name, quality_name): # Await the async function
...
async def confirm_publish_callback(client: Client, callback_query: CallbackQuery):
    ...
    if await publish_series(series_key): # Await the async function
