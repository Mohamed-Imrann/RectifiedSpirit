...
async def download_and_upload_poster(client, message):
    try:
        ...
        # Try copying to log channel (optional)
        try:
            if LOG_CHANNEL:
...
async def _optimize_poster_to_jpeg(client, message, max_w=1080, max_h=1620, quality=94, max_bytes=2_500_000):
    """
    Download poster photo -> resize -> high-quality JPEG (no upscale) -> return BytesIO (ready to upload).
    If Pillow is not installed, returns None and caller will fallback.
    """
    if Image is None:
        return None
    ...
    bio = BytesIO()
    await client.download_media(message, file_name=bio)
    bio.seek(0)
    ...
    q = quality
    while out.getbuffer().nbytes > max_bytes and q > 80:
        q -= 3
        out = BytesIO()
...
def _extract_media_file_id(message):
    """Pyrogram v2 safe: message.photo is a Photo object (not list)."""
    if message.photo:
...
async def _store_in_log_channel(client, message, file_id: str):
    """
    ✅ Photo posters: re-upload optimized HQ JPEG to LOG_CHANNEL and return stored file_id.
    ✅ Video/Document: copy as-is to LOG_CHANNEL.
    Fallback: return original file_id.
    """
    try:
        if not LOG_CHANNEL:
            return file_id

        # Photo -> optimize & re-upload (HQ, consistent size)
        if getattr(message, "photo", None):
            poster_bio = await _optimize_poster_to_jpeg(client, message)
            if poster_bio:
                sent = await client.send_photo(chat_id=int(LOG_CHANNEL), photo=poster_bio)
                if sent.photo:
                    return sent.photo.file_id

        # Others -> copy original (video/document)
        copied = await client.copy_message(
            chat_id=int(LOG_CHANNEL),
            from_chat_id=message.chat.id,
            message_id=message.id
        ...
        logger.warning(f"Log channel store failed, using original file_id: {e}")
...
async def process_poster_input(client: Bot, message: Message, poster_type: str, mode: str):
    ...
    series_key = temp_admin_data.get(user_id, {}).get("current_series_key")
    if not series_key:
        await message.reply_text("❌ Session expired. Please /editseries again.")
        return
    ...
    try:
        if poster_type == "series":
            series_collection.update_one(
                {"_id": series_key},
                {"$set": {"poster_file_id": file_id}}
            )

        elif poster_type == "language":
            language_name = temp_admin_data.get(user_id, {}).get("current_language")
            if not language_name:
                await message.reply_text("❌ Language not selected. Go back and select language again.")
                return

            series = series_collection.find_one({"_id": series_key}) or {}
            ...
            language_name = temp_admin_data.get(user_id, {}).get("current_language")
            season_name = temp_admin_data.get(user_id, {}).get("current_season")
            if not language_name or not season_name:
                await message.reply_text("❌ Language/Season not selected. Go back and select again.")
                return
        ...
        # ✅ refresh main UI message if available
        main_message_id = temp_admin_data.get(user_id, {}).get("main_message_id")
        try:
            series_data = get_series_by_key(series_key)
            if series_data and main_message_id:
                await send_series_details_message(client, user_id, series_data, main_message_id, mode)
        except Exception as e:
            logger.warning(f"Failed to refresh UI after poster update: {e}")

        await message.reply_text("✅ Poster updated successfully!")

    except Exception as e:
        logger.error(f"Poster save failed: {e}")
        await message.reply_text(f"❌ Failed to process the poster: {e}")
