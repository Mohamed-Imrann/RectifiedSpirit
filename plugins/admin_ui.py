...
async def handle_media_message(client: Client, message: Message):
    ...
    user_id = message.from_user.id
    chat_id = message.chat.id
    logger.info(f"Received media message {message.id} from user {user_id} in chat {chat_id}")
    
    if message.chat.type != enums.ChatType.PRIVATE:
        # In groups, we don't process media for admin UI, so we return
        logger.info(f"Media message in group {chat_id}, ignoring")
        return
    ...
    logger.info(f"Ignoring media message from user {user_id}")
    return
...
async def callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    if data.startswith("newui_"):
        if user_id in ADMINS:
            logger.info(f"Admin UI callback from user {user_id}")
            async with get_admin_lock(user_id):
                await newui_callback_handler(client, callback_query)
        else:
            logger.warning(f"Unauthorized admin UI callback from non-admin user {user_id}")
