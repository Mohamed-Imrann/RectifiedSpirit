...
async def on_join_request(client: Client, join_request: ChatJoinRequest):
    ...
    try:
        pend = await get_pending(int(user_id))
    except Exception:
        pend = None
    ...
    # ✅ user joined/requested => advance step
    try:
        ok = await check_and_advance_if_joined(client, int(user_id))
    except Exception:
        ok = False
    ...
    # ✅ pending key extract (tk:token OR direct link key)
    send_key = (
        pend.get("key")
        or pend.get("temp_key")
        or pend.get("data_key")
        or pend.get("pending_key")
    )
    ...
    try:
        sent = await sendseries(client, f"{user_id}:joinreq", send_key)
        if sent:
            try:
                await clear_pending(int(user_id))
            except Exception:
...
@Bot.on_chat_join_request()
async def _auto_send_on_join_request(client: Bot, join_request: ChatJoinRequest):
    """
    ✅ Join-request வந்ததும்:
    - pending irundha
    - fsub step joined/requested ok na
    - sendseries மூலம் files auto send
    """
    user_id = join_request.from_user.id

    # get pending request (saved earlier when user clicked quality)
    try:
        pend = await get_pending(int(user_id))
    except Exception:
        pend = None
    ...
    # ✅ If user is joined/requested for required channel -> ok
    try:
        ok = await check_and_advance_if_joined(client, int(user_id))
    except Exception:
        ok = False
    ...
    # ✅ pending key: usually "tk:<token>"
    # some DBs store under different keys, so check safely
    send_key = (
        pend.get("key")
        or pend.get("temp_key")
        or pend.get("pending_key")
    ...
    try:
        ...
        logging.getLogger(__name__).error(f"auto send on join_request failed: {e}")
        return
