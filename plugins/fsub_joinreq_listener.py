...
async def on_join_request_handler(client, join_request):
    try:
        ...
        # ✅ allow any of your fsub chats (chat1/chat2/chat3) request
        # so wrong required_chat_id won't block
        required_chats = []
        try:
            c1 = await dbj.get_fsub_chat1()
            if c1 and c1.get("chat_id"): required_chats.append(int(c1["chat_id"]))
        except: pass
        try:
            c2 = await dbj.get_fsub_chat2()
            if c2 and c2.get("chat_id"): required_chats.append(int(c2["chat_id"]))
        except: pass
        if hasattr(dbj, "get_fsub_chat3"):
            try:
                c3 = await dbj.get_fsub_chat3()
                if c3 and c3.get("chat_id"): required_chats.append(int(c3["chat_id"]))
            except: pass
