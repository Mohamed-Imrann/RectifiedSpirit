...
async def newseries_panel(client: Client, message):
    ...
    # if poster fetched -> replace panel with photo panel
    if ok:
...
async def adm_addlang(client: Client, cq):
    ...
    askmsg = await cq.message.reply_text("🌐 Language name anuppu (ex: Multi Audio / Tamil):")
    try:
        ans = await wait_user_message(cq.message.chat.id, cq.from_user.id, timeout=180)
    except Exception:
        return await askmsg.edit_text("❌ Timeout. Retry.")

    lang = (ans.text or "").strip()
    if not lang:
        return await askmsg.edit_text("❌ Empty. Retry.")
...
async def adm_addseason(client: Client, cq):
    ...
    askmsg = await cq.message.reply_text("📦 Season name anuppu:")
    try:
        ans = await wait_user_message(cq.message.chat.id, cq.from_user.id, timeout=180)
    except Exception:
        return await askmsg.edit_text("❌ Timeout. Retry.")

    season = (ans.text or "").strip()
    if not season:
        return await askmsg.edit_text("❌ Empty. Retry.")
...
async def adm_addquality(client: Client, cq):
    ...
    askmsg = await cq.message.reply_text("🎞 Quality anuppu (ex: 720p / 1080p):")
    try:
        ans = await wait_user_message(cq.message.chat.id, cq.from_user.id, timeout=180)
    except Exception:
        return await askmsg.edit_text("❌ Timeout. Retry.")

    quality = (ans.text or "").strip()
    if not quality:
        return await askmsg.edit_text("❌ Empty. Retry.")
...
async def adm_upload(client: Client, cq):
    ...
    first_m = await wait_user_message(cq.message.chat.id, cq.from_user.id, timeout=180)
    chat_id, first_id = _forward_info(first_m)
    if not chat_id:
        return await info.edit_text("❌ First forward varala. Source message forward pannunga.")

    await info.edit_text("🔗 ✅ First ok.\n\n➡️ Ippo **LAST file message** ah forward pannunga.")

    last_m = await wait_user_message(cq.message.chat.id, cq.from_user.id, timeout=180)
    chat_id2, last_id = _forward_info(last_m)
    if not chat_id2 or chat_id2 != chat_id:
        return await info.edit_text("❌ Last forward wrong. Same channel/group-la irundhu forward pannunga.")
