...
async def safe_set_panel(cq, text: str, reply_markup=None):
    ...
    If edit fails (message changed, can't edit caption, etc), fallback to sending new.
    """
    try:
        if cq.message.photo:
...
async def adm_home(client, cq):
    ...
    # safest: edit if possible else send new
